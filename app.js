/* ============================================================
   Chittoor Police — PPP Public Feedback Dashboards
   ...
   This file drives TWO independent dashboard instances from one
   shared engine — CAW (Crime Against Women) and GANJA (Narcotics) —
   selected via the top "PPP SURVEY" switcher. Each instance has its
   own DOM (id-prefixed), its own localStorage keys, its own /data
   folder on GitHub, and its own seed data, but otherwise behaves
   identically.
   ============================================================ */
...
// Shared admin session across both surveys — one login unlocks Data & Upload
// on both CAW and GANJA. SHA-256 hash of the admin password — never store the
// plain password in this file. This is the same password as the original
// CAW dashboard ("Chittoor@CAW2026") — CHANGE THIS before publishing if you
// want a new one (see README): regenerate with sha256("your-new-password").
const ADMIN_KEY = 'ppp_admin_v1';
const ADMIN_HASH = 'b698a3e95e7bd4abd43a80ba96a1139ea1d4ad7a7384049f066a98516af6b099';

/* ---------------- shared pure helpers (no DOM / no per-instance state) ---------------- */
...
/* ============================================================
   FACTORY — creates one fully independent dashboard instance.
   cfg: { prefix, storagePrefix, dataFolder, seedMonth, defaultSurveyTitle }
   ============================================================ */
function createSurveyApp(cfg){
  const root = document.getElementById('dash-'+cfg.prefix);
  const $id = (suffix)=> cfg.prefix + '-' + suffix;
  const gid = (suffix)=> document.getElementById($id(suffix));

  const LS_KEY = cfg.storagePrefix + '_dashboard_months_v1';
  const HIDDEN_KEY = cfg.storagePrefix + '_dashboard_hidden_v1';
  ...
  function saveLocalMonths(list){ try{ localStorage.setItem(LS_KEY, JSON.stringify(list)); }catch(e){} }
  function loadHiddenMonths(){
    try{ const raw = localStorage.getItem(HIDDEN_KEY); return raw ? JSON.parse(raw) : []; }catch(e){ return []; }
  }
  function saveHiddenMonths(list){ try{ localStorage.setItem(HIDDEN_KEY, JSON.stringify(list)); }catch(e){} }
  ...
  async function fetchManifestMonths(){
    ...
    if(!info) return out;
    try{
      const apiUrl = `https://api.github.com/repos/${info.owner}/${info.repo}/contents/${cfg.dataFolder}`;
  ...
  async function boot(){
    ...
    const useSeed = (visibleGithubMonths.length === 0 && localMonths.length === 0) ? cfg.seedMonth : null;
  ...
  function setSource(ok, text){
    const dot = gid('srcdot');
    dot.className = 'srcdot' + (ok===true?' ok':ok==='warn'?' warn':'');
    gid('srctext').textContent = text;
  }
  ...
  function toast(msg){
    ...
    t.__toastTimer = setTimeout(()=>t.classList.remove('show'), 3200);
  }

  /* ---------------- excel parsing (instance-specific: needs STATE for short labels) ---------------- */
  function parseExcelBufferToMonth(buf, filename, monthKeyOverride){
    ...
    return {
      ...
      reportAsOn: '',
      sourceFile: filename,
      surveyTitle: cfg.defaultSurveyTitle,
  ...
  function hideGithubMonth(key){
    if(!confirm('Remove '+monthLabel(key)+' from this dashboard?\n\nThis only hides it in this browser. To delete it from the published site for everyone, also delete that month\'s .xlsx file from the '+cfg.dataFolder+'/ folder in your GitHub repo, then commit & push.')) return;
    ...
    paintChrome();
    renderAll();
    toast('Hidden '+monthLabel(key)+' in this browser. Delete that month\'s .xlsx from the '+cfg.dataFolder+'/ folder in your repo to remove it everywhere.');
  }
  ...
  function askMonth(guess, filename){
    return new Promise(resolve=>{
      const bg = gid('monthModalBg');
      const sel = gid('mmMonth');
      gid('mmSub').textContent = `File: ${filename}. Confirm the reporting month for this snapshot.`;
      sel.innerHTML = '';
      const now = new Date();
      const opts = [];
      for(let i=0;i<24;i++){
      ...
      const save = gid('mmSave');
      const cancel = gid('mmCancel');
  ...
  function periodStats(monthsList){
    ...
    const perQ = [];
    for(let qi=0; qi<nq; qi++){ perQ.push({ pos: weightedMean(rows, qi, 'pos'), neg: weightedMean(rows, qi, 'neg') }); }
  ...
  function stationRollup(monthsList){
    ...
    for(const m of monthsList){
      for(const r of m.rows){
        ...
        byLoc[key].mandal = r.mandal || byLoc[key].mandal;
      }
    }
    const list = Object.values(byLoc).map(st=>{
      const perQ = [], perQNeg = [];
      for(let qi=0; qi<nq; qi++){ perQ.push(weightedMean(st.rows, qi, 'pos')); perQNeg.push(weightedMean(st.rows, qi, 'neg')); }
  ...
  function paintChrome(){
    ...
    const desc = monthsDesc();
    const latest = desc[0];
    gid('s-ps').textContent = latest ? new Set(latest.rows.map(r=>r.location)).size : '—';
    gid('s-q').textContent = nq || '—';
    gid('s-mo').textContent = STATE.months.length || '—';
    gid('h-asof').textContent = latest ? `Latest report: ${latest.reportLabel||monthLabel(latest.month)}` : 'Report as on: —';

    const visibleTabs = TABS.filter(t=> t.id!=='data' || STATE.isAdmin);
    const tabrow = gid('tabrow');
  ...
  async function switchTab(id){
    ...
    STATE.activeTab = id;
    root.querySelectorAll('.tabbtn').forEach(b=> b.classList.toggle('active', b.dataset.tab===id));
    root.querySelectorAll('.panel').forEach(p=> p.classList.remove('active'));
    gid('p-'+id).classList.add('active');
    renderTab(id);
  }
  ...
  function renderTab(id){
    if(id==='data' && !STATE.isAdmin){
      gid('p-data').innerHTML = `<div class="card"><div class="emptynote">
  ...
  function emptyState(){
    const cta = STATE.isAdmin
      ? `<div style="margin-top:14px"><button class="btn primary" onclick="${cfg.prefix}__switchTab('data')">Go to Data &amp; Upload</button></div>`
      : '';
    return `<div class="card"><div class="emptynote">
      <div class="big">📊</div>
      <h3>No report data published yet</h3>
      <p style="margin-top:6px">The monthly report is uploaded by district admins. Please check back soon.</p>
      ${cta}
    </div></div>`;
  }
    ...
    el.innerHTML = `
      <div class="card">
        <div class="ch" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
          ...
          <select id="${$id('rankPeriodSel')}">${periodOpts.map(([k,l])=>`<option value="${k}" ${k===STATE.rankPeriod?'selected':''}>${l}</option>`).join('')}</select>
        </div>
        <div class="cb"><div class="chart-box" id="${$id('rankChartBox')}" style="height:520px"><canvas id="${$id('c-rank')}"></canvas></div></div>
      </div>
      <div class="card">
        <div class="ch"><h3>Full Ranking Table</h3></div>
        <div class="cb" style="overflow-x:auto" id="${$id('rankTableWrap')}"></div>
      </div>
    `;
    gid('rankPeriodSel').onchange = (e)=>{ STATE.rankPeriod = e.target.value; drawRankings(); };
    drawRankings();
  }
    ...
    el.innerHTML = `
      <div class="card">
        <div class="ch" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
          ...
          <select id="${$id('heatMonthSel')}">${opts}</select>
        </div>
        <div class="cb" style="overflow-x:auto" id="${$id('heatWrap')}"></div>
      </div>
    `;
    gid('heatMonthSel').onchange = (e)=>{ STATE.heatmapMonth = e.target.value; drawHeatmap(); };
    drawHeatmap();
  }
  ...
  function drawHeatmap(){
    ...
    const qShort = questionLabels(true);
    if(!m){ gid('heatWrap').innerHTML = `<div class="emptynote">No data.</div>`; return; }
    ...
    el.innerHTML = `
      <div class="card">
        <div class="ch" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
          ...
          <select id="${$id('detailSel')}" style="min-width:260px">${opts}</select>
        </div>
        <div class="cb" id="${$id('detailBody')}"></div>
      </div>
    `;
    gid('detailSel').onchange = (e)=>{ STATE.detailPS = e.target.value; drawDetail(); };
    drawDetail();
  }

  function drawDetail(){
    const loc = STATE.detailPS;
    const body = gid('detailBody');
    ...
    body.innerHTML = `
      ...
      <div class="grid g2">
        <div class="card" style="margin-bottom:0">
          ...
          <div class="cb"><div class="chart-box" style="height:300px"><canvas id="${$id('c-det-bars')}"></canvas></div></div>
        </div>
        <div class="card" style="margin-bottom:0">
          <div class="ch"><h3>Monthly Trend for this Station</h3></div>
          <div class="cb"><div class="chart-box" style="height:300px"><canvas id="${$id('c-det-trend')}"></canvas></div></div>
        </div>
      </div>
      <div class="card">
  ...
  function renderData(){
    ...
    const rows = desc.map(m=>{
      ...
      return `<tr><td><b>${m.reportLabel||monthLabel(m.month)}</b></td><td class="small">${m.sourceFile||'—'}</td>
        ...
        <td class="right">
          <button class="linkbtn" onclick="${cfg.prefix}__downloadMonthJson('${m.month}')">Export JSON</button>
          ${m.__source==='local' ? ` &nbsp;·&nbsp; <button class="linkbtn" style="color:var(--down)" onclick="${cfg.prefix}__deleteMonth('${m.month}')">Remove</button>` : ''}
          ${m.__source==='github' ? ` &nbsp;·&nbsp; <button class="linkbtn" style="color:var(--down)" onclick="${cfg.prefix}__hideGithubMonth('${m.month}')">Remove</button>` : ''}
        </td></tr>`;
    }).join('');
    ...
    const hiddenRow = hiddenKeys.length ? `
      <div class="card">
        ...
        <div class="cb small">
          ${hiddenKeys.map(k=>`<div style="margin-bottom:6px">${monthLabel(k)} &nbsp;·&nbsp; <button class="linkbtn" onclick="${cfg.prefix}__unhideGithubMonth('${k}')">Restore</button></div>`).join('')}
        </div>
      </div>` : '';
      ...
      <div class="card">
        ...
        <div class="cb">
          <div class="dropzone" id="${$id('dropzone')}">
            ...
            <div class="dzsub">Expected format: Mandal / Location Wise Analysis Report (.xlsx) — same layout every month, with Mandal, Location, Total and one +ve(%)/-ve(%) column pair per question.</div>
          </div>
          <input type="file" id="${$id('fileInput')}" accept=".xlsx,.xls,.csv" multiple hidden>
      ...
      <div class="card">
        ...
        <div class="cb">
          ...
          <ol style="margin:12px 0 0 20px;color:var(--muted);font-size:13.5px;line-height:1.9">
            <li>In your GitHub repo, open the <code>${cfg.dataFolder}</code> folder and click <b>Add file → Upload files</b>.</li>
            <li>Upload the month's <code>.xlsx</code> report as-is — keep the month name and year somewhere in the filename.</li>
            <li>Commit. GitHub Pages updates within a minute or two — the dashboard lists the <code>${cfg.dataFolder}</code> folder and reads every Excel file it finds automatically.</li>
          </ol>
  ...
  /* ---------------- wire up chrome buttons + expose inline-onclick globals ---------------- */
  gid('btnRefresh').onclick = ()=> boot();
  gid('btnGoUpload').onclick = ()=> switchTab('data');
  const btnLogoutInit = gid('btnAdminLogout');
  if(btnLogoutInit) btnLogoutInit.onclick = ()=> adminLogout();

  window[cfg.prefix+'__switchTab'] = switchTab;
  window[cfg.prefix+'__downloadMonthJson'] = downloadMonthJson;
  window[cfg.prefix+'__deleteMonth'] = deleteMonth;
  window[cfg.prefix+'__hideGithubMonth'] = hideGithubMonth;
  window[cfg.prefix+'__unhideGithubMonth'] = unhideGithubMonth;

  return { boot, renderAll, switchTab };
}

/* ============================================================
   INSTANTIATE both surveys + top-level survey switcher
   ============================================================ */
const cawApp = createSurveyApp({
  prefix: 'caw',
  storagePrefix: 'caw',
  dataFolder: 'data',
  seedMonth: window.SEED_MONTH_CAW,
  defaultSurveyTitle: 'PS WISE PUBLIC FEEDBACK ON CRIME AGAINST WOMEN',
});
const gjApp = createSurveyApp({
  prefix: 'gj',
  storagePrefix: 'ganja',
  dataFolder: 'data-ganja',
  seedMonth: window.SEED_MONTH_GJ,
  defaultSurveyTitle: 'PS WISE PUBLIC FEEDBACK ON GANJA (NARCOTICS) MENACE',
});
const SURVEY_APPS = { caw: cawApp, gj: gjApp };
const bootedSurveys = new Set();

function showSurvey(key){
  document.querySelectorAll('.dash-instance').forEach(d=> {
    d.style.display = d.dataset.surveyRoot===key ? '' : 'none';
  });
  document.querySelectorAll('.surveytab').forEach(b=> b.classList.toggle('active', b.dataset.survey===key));
  if(!bootedSurveys.has(key)){
    bootedSurveys.add(key);
    SURVEY_APPS[key].boot();
  } else {
    // re-render so Chart.js recomputes canvas size now that the container is visible again
    SURVEY_APPS[key].renderAll();
  }
}

document.querySelectorAll('.surveytab').forEach(b=> b.onclick = ()=> showSurvey(b.dataset.survey));

// boot the initially-visible survey (CAW) immediately; GANJA boots lazily on first click
bootedSurveys.add('caw');
cawApp.boot();
